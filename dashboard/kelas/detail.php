...
<body>
    <div id="mySidenav" class="sidenav">
        <div class="link">
            ...
            <a href="./" class="icon-a active <?= $role != 1 ? 'hidden' : '' ?>"><i class="bx bx-chair icons">&nbsp;&nbsp;</i><b class="link_nama">Kelas</b></a>
    ...
    <div class="content">
        <div id="main">
            <div class="head">
                <div class="col-div-6">
                    <span class="nav">Data Kelas</span>
                </div>
                <div class="search">
                    <input type="search" name="" id="search-input" placeholder="Cari data siswa" data-kelas="<?= $_GET['id'] ?>">
            ...
            $limit = 10;
            $kelas = $_GET['id'];
            $query_total = "SELECT COUNT(*) AS total_data FROM siswa WHERE id_kelas = '$kelas' ";
            ...
            $offset = ($page - 1) * $limit;
            $no = 1;
            // $query_page = "SELECT * FROM kelas INNER JOIN siswa INNER JOIN jurusan ON kelas.id_jurusan = jurusan.id_jurusan LEFT JOIN guru ON kelas.wali_kelas = guru.id_guru WHERE id_kelas = '$kelas' ORDER BY id_kelas DESC LIMIT $limit OFFSET $offset";
            $query_page = "SELECT * FROM siswa INNER JOIN kelas ON siswa.id_kelas = kelas.id_kelas INNER JOIN jurusan ON kelas.id_jurusan = jurusan.id_jurusan WHERE siswa.id_kelas = '$kelas' LIMIT $limit OFFSET $offset";
            ...
            ?>
                <div class="pagination">
                    <div class="pagination-link">
                        <a href="?id=<?= $_GET['id'] ?>&page=<?= $prev ?>"><i class='bx bx-skip-previous'></i></a>
                        <div class="pg-num">
                            <?php for ($i = 1; $i <= $total_page; $i++) { ?>
                                <?php if ($i == $page) { ?>
                                    <a href="?id=<?= $_GET['id'] ?>&page=<?= $i ?>" class="pagi-active"><?= $i ?></a>
                                <?php } else { ?>
                                    <a href="?id=<?= $_GET['id'] ?>&page=<?= $i ?>"><?= $i ?></a>
                                <?php } ?>
                            <?php } ?>
                        </div>
                        <a href="?id=<?= $_GET['id'] ?>&page=<?= $next ?>"><i class='bx bx-skip-next'></i></a>
                    </div>
                </div>
                <div class="col-div-8">
                    <div class="box-8">
                        <div class="content-box">
                            <div class="tb-tittle">
                                <a href="export.php?id=<?= $_GET['id'] ?>" class="export-btn"><i class='bx bx-export'></i></a>
                                <h1>Data Siswa Kelas <?= $data_kelas['tingkatan'] ?> <?= $data_kelas['jurusan'] ?> <?= $data_kelas['nama_kelas'] ?></h1>
                                <span></span>
                            </div>
                            <table>
                                <tr>
                                ...
                                ?>
                                    <tr>
                                        ...
                                        <td><?= $s['nis'] ?></td>
                                        <td><?= $s['nama'] ?></td>
                                        <td><?= $s['id_kelas'] == ' ' ? 'Lulus' : $s['tingkatan'] . " " .  $s['jurusan'] . " " .  $s['nama_kelas'] ?></td>
                                        <td><?= $s['status'] ?></td>
                                        <td>
                                            <div class="aksi-field">
                                                <a href="../users/detail/index.php?data=siswa&nis=<?= $s['nis'] ?>" class="detail-btn"><i class='bx bxs-user-detail'></i></a>
                                                <a href="../users/aksi/edit.php?data=siswa&nis=<?= $s['nis'] ?>" class="edit-btn"><i class='bx bxs-edit'></i></a>
                                                <a href="../users/aksi/hapus_siswa.php?id=<?= $s['id_siswa'] ?>" class="delete-btn" onclick="return confirm('Anda yakin untuk menghapus?')"><i class='bx bxs-trash'></i></a>
                                            </div>
                                        </td>
                                    </tr>
                                <?php } ?>
